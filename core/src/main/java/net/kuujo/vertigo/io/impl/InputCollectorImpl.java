...
public class InputCollectorImpl implements InputCollector {
  ...
  public InputCollectorImpl(Vertx vertx, InputInfo info) {
    this.vertx = vertx;
    this.info = info;
    this.log = LoggerFactory.getLogger(String.format("%s-%s-%d", InputCollectorImpl.class.getName(), info.instance().component().name(), info.instance().number()));
  }
  ...
  public InputCollector open(final Handler<AsyncResult<Void>> doneHandler) {
    ...
    tasks.runTask((task) -> {
      if (!started) {
        ...
        for (final InputPortInfo port : info.ports()) {
