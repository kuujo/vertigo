...
@LocalType
@ClusterType
public class RedisQueue<T> implements AsyncQueue<T> {
  private final String name;
  private final String address;
  private final Vertx vertx;

  @Factory
  public static <T> RedisQueue<T> factory(String name, String address, Vertx vertx) {
  ...
  public void contains(Object value, Handler<AsyncResult<Boolean>> resultHandler) {
    throw new UnsupportedOperationException("Redis queue contains operation not supported.");
  }
    ...
    vertx.eventBus().sendWithTimeout(address, message, 30000, new Handler<AsyncResult<Message<JsonObject>>>() {
      ...
      public void handle(AsyncResult<Message<JsonObject>> result) {
        if (result.failed()) {
          ...
          new DefaultFutureResult<T>(new VertigoException(result.result().body().getString("message"))).setHandler(resultHandler);
        } else if (result.result().body().<T>getValue("value") == null) {
