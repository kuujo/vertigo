...
import net.kuujo.vevent.eventbus.TimeoutException;
...
public class ReliableExecutor extends AbstractExecutor implements Executor {
  ...
  public ReliableExecutor(NetworkContext context, Vertx vertx) {
    super(context, vertx);
    this.vertx = vertx;
    createUniqueId();
    init();
  ...
  public ReliableExecutor(NetworkContext context, Vertx vertx, EventBus eventBus) {
  ...
  protected void outputReceived(Message<JsonObject> message) {
    ...
    if (id == null) {
      ...
      JsonObject messageBody = body.getObject("body");
      if (messageBody != null) {
