 ...
 */
public class LocalVitis implements Vitis {

  protected static final String STEM_VERTICLE_CLASS = SeedVerticle.class.getName();
  ...
  public LocalVitis(Vertx vertx, Container container) {
    this.vertx = vertx;
  ...
  private void deployLocalStem(VineDefinition definition, Handler<AsyncResult<String>> handler) {
    ...
    if (stemAddress != null) {
      ...
      container.deployVerticle(STEM_VERTICLE_CLASS, new SeedContext(new JsonObject().putString("address", address)).serialize(), new Handler<AsyncResult<String>>() {
        @Override
        public void handle(AsyncResult<String> result) {
          if (result.succeeded()) {
            stemAddress = address;
  ...
  private class RecursiveExecutor {
     ...
     */
    private class RecursiveSeedExecutor extends RecursiveContextExecutor<NodeContext> {

      public RecursiveSeedExecutor(Collection<NodeContext> contexts) {
        super(contexts);
      }

      @Override
      protected void doExecute(NodeContext context, Handler<AsyncResult<Boolean>> resultHandler) {
