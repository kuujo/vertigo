...
/**
 * A reliable embedded stream feeder verticle.
 *
 * @author Jordan Halterman
 */
public abstract class ReliableEmbeddedStreamFeederVerticle extends Verticle {

  private ReliableEmbeddedStreamFeeder feeder;

  @Override
  public void start() {
    feeder = new ReliableEmbeddedStreamFeeder(vertx, container, new WorkerContext(container.config()));
    feeder.feedHandler(new Handler<ReliableEmbeddedStreamFeeder>() {
      @Override
      public void handle(ReliableEmbeddedStreamFeeder feeder) {
        feed(feeder);
      }
    });
   ...
   */
  protected abstract void feed(ReliableEmbeddedStreamFeeder feeder);
